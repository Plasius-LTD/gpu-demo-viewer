...
test("wavefront demo uses the renderer mesh BVH path", () => {
  ...
  assert.match(source, /meshes:\s*createWavefrontDemoModelMeshes\(\)/);
  ...
  assert.doesNotMatch(source, /createQuadMesh/);
  assert.doesNotMatch(source, /materialKind:\s*"emissive"/);
  assert.doesNotMatch(source, /materialKind:\s*"transparent"/);
  assert.doesNotMatch(source, /sceneObjects/);
  assert.doesNotMatch(source, /id:\s*100/);
  assert.doesNotMatch(source, /id:\s*101/);
  assert.doesNotMatch(source, /id:\s*102/);
  assert.doesNotMatch(source, /id:\s*103/);
  assert.doesNotMatch(source, /id:\s*104/);
  assert.doesNotMatch(source, /id:\s*105/);
